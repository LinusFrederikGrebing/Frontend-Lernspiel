<template>
  <v-container>
    <v-card elevation="24" outlined shaped width="120em" height="5em">
      <h1 class="section_header font-weight-black">
        - {{ $t("navigation.tutorial") }} -
      </h1>
    </v-card>
    <v-card elevation="10" outlined width="100%">
      <div class="d-lg-flex justify-lg-center tutorial text-center">
        ...
        <div class="editor-in-tutorial mr-16">
          ...
          ></CodeEditor>
          <v-btn id="validate-btn" color="warning" depressed elevation="2">{{
            $t("codeInput.validate")
          }}</v-btn>
          <v-btn id="finished-btn" color="success" depressed elevation="2">{{
            $t("codeInput.run")
          }}</v-btn>
        </div>
      </div>
...
export default {
  ...
  methods: {
    tutorialAnimation() {
      ...
      for (let i = 0; i <= this.codeToRun.length; i++) {
        this.timerTutorialAnimation = setTimeout(
          () => (this.actualCodeToRun = this.codeToRun.substring(0, i)),
          i * 250
        );
      }
      this.cursorTutorialAnimation = setTimeout(
        () => this.cursorToFinishedAnimation(),
        3500
      );
    },
    ...
    cursorToFinishedAnimation() {
      ...
      let y =
        parseInt(finishedButton.offsetHeight) / 2 -
        headerHeight -
        scrollY +
        yOffset;
      ...
      while (
        finishedButton &&
        !isNaN(finishedButton.offsetLeft) &&
        !isNaN(finishedButton.offsetTop)
      ) {
    ...
    cursorToValidateAnimation() {
      ...
      let y =
        parseInt(validateButton.offsetHeight) / 2 -
        headerHeight -
        scrollY +
        yOffset;
      if (
      ...
      while (
        validateButton &&
        !isNaN(validateButton.offsetLeft) &&
        !isNaN(validateButton.offsetTop)
      ) {
    ...
    showTutorialPopup() {
      ...
      let y =
        parseInt(codeEditor.offsetHeight) / 2 -
        headerHeight -
        scrollY +
        yOffset;
      while (
      ...
      if (gridContainer.classList.contains("cardgrid_container_xs"))
        alert.style.width = "70vw";
      gsap.to("#alert-for-op-tutorial", {
        duration: 0,
        x: x - alert.width / 2,
        y: y - alert.height / 2,
        visibility: "visible",
    ...
    paintTutorialField(first, second) {
      let element = document.getElementById(
        "xTutorial" + first + "yTutorial" + second
      );
